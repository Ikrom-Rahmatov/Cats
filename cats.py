...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_paragraphs = [x for x in paragraphs if select(x) == True]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def checking(paragraph):
        lower_p = lower(remove_punctuation(paragraph)).split()
        words = [x for x in topic if x in lower_p]
        if len(words) == 0 :
            return False
        return True
    return checking
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        difference = [diff_function(typed_word, x , limit) for x in valid_words]
        if min(min(difference), limit) in difference:
    ...
    if len(start) == 1 and len(goal) == 1:
        ...
        if len(start) > len(goal):
            if len(start)-len(goal) > limit:
                return limit + 1
            return len(start) - len(goal) + feline_flips(start[:len(goal)], goal, limit)
        elif len(goal) > len(start):
            if len(goal)-len(start)> limit:
                return limit+1
            return len(goal)-len(start) + feline_flips(start, goal[:len(start)], limit)
        else:
            if start[0] == goal[0]:
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[x-t[t.index(x)-1] for x in t][1:] for t in times_per_player]
...
def fastest_words(match):
    ...
    for i in range(len(get_words(match))):
        times = []
        for x in range(len(get_times(match))):
            times = times + [time(match, x, i)]
        who_typed_fast = who_typed_fast + [times.index(min(times))]
    words = [[]] * len(get_times(match))
    for x in get_words(match):
        words[who_typed_fast[get_words(match).index(x)]] = words[who_typed_fast[get_words(match).index(x)]] + [x]
    return words
    # END PROBLEM 10
